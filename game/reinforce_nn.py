...
class Nn:
    def __init__(self, restore_call = False):
        ...
        # Variables
        self.x = tf.placeholder(tf.float32, [None,None, 9*9])
...
#
    ...
    def res_block(self,inputs):
        conv_res1 = Convolution2D(inputs, (9, 9), 64, 64, (3, 3), activation='none')
        conv_res1_bn = self.batch_norm(conv_res1.output(), 64, self.phase_train)
        conv_res1_out = tf.nn.relu(conv_res1_bn)
        conv_res2 = Convolution2D(conv_res1_out, (9, 9), 64, 64, (3, 3), activation='none')
...
# Create the model
    def inference(self,x, y_):
        x_image = tf.reshape(x, [-1, 9, 9, 5])
        
        with tf.variable_scope('conv_1'):
            conv1 = Convolution2D(x, (9, 9), 5, 64, (5, 5), activation='none')
...
#        pool1_out = pool1.output()
        ...
        with tf.variable_scope('conv_2'):
            conv2 = Convolution2D(res_block_out9, (9, 9), 64, 2, (1, 1), 
                                                              activation='none')
            ...
            pool2_flat = tf.reshape(conv2_out, [-1, 9*9*2])
        
        # with tf.variable_scope('fc1'):
            # fc1 = FullConnected(pool2_flat, 9*9*2, 1024)
            # fc1_out = fc1.output()
            # fc1_dropped = tf.nn.dropout(fc1_out)
        
        self.y_pred = ReadOutLayer(pool2_flat, 9*9*2, 82).output()
